...
const OtherSetup = ({ onBack, settings, onUpdate, folderName, bookName, pages = [] }) => {
  ...
  const [deleteImageAlert, setDeleteImageAlert] = useState({ isOpen: false, index: null });
  ...
  const confirmDeleteImage = (index) => {
    setDeleteImageAlert({ isOpen: true, index });
    setOpenContextMenu(null);
  };

  const deleteImage = async () => {
    const index = deleteImageAlert.index;
    if (index === null) return;
    const img = slideshowImages[index];
    if (!img) return;

    setDeleteImageAlert({ isOpen: false, index: null });
  ...
  return (
    <div className="flex flex-col h-full bg-white relative overflow-visible">
      ...
      >
        ...
        <AlertModal
          isOpen={deleteImageAlert.isOpen}
          onClose={() => setDeleteImageAlert({ isOpen: false, index: null })}
