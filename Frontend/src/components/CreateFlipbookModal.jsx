...
const CreateFlipbookModal = ({ isOpen, onClose, onUpload, onTemplate, initialView = 'upload', initialTemplateId = 'corporate', existingFlipbooks = [], initialFiles = null }) => {
  ...
  const hasUserEditedNameRef = React.useRef(false);
  ...
  const isAutoGeneratedName = (name) => {
    ...
    if (!hasUserEditedNameRef.current) return true;
    if (name === initialFlipbookNameRef.current || name === initialFlipbookName) return true;
    return /^(PDF|Word|PPT|Flipbook)_(Flipbook_)?\d+$/i.test(name.trim());
  };
  ...
  const handleFileChange = async (event) => {
    ...
    // Immediately update flipbook name if currently using a default name
    if (uploadedFiles.length === 0 && isAutoGeneratedName(flipbookName)) {
      const firstType = getOfficeDocType(validFiles[0].name);
      ...
      initialFlipbookNameRef.current = newDefName;
    }
    ...
    try {
      ...
      if (acceptedFiles.length > 0) {
        if (isAutoGeneratedName(flipbookName)) {
          const firstType = (uploadedFiles[0] || acceptedFiles[0]).docType;
          const prefix = firstType === 'word' ? 'Word_Flipbook_' : firstType === 'powerpoint' ? 'PPT_Flipbook_' : 'PDF_Flipbook_';
          const newDefName = `${prefix}${getFormattedDateTime()}`;
          setFlipbookName(newDefName);
          setInitialFlipbookName(newDefName);
    ...
    setUploadedFiles(prev => {
      ...
      if (isAutoGeneratedName(flipbookName)) {
        if (remaining.length > 0) {
          const firstType = remaining[0].docType;
          const prefix = firstType === 'word' ? 'Word_Flipbook_' : firstType === 'powerpoint' ? 'PPT_Flipbook_' : 'PDF_Flipbook_';
          const newDefName = `${prefix}${getFormattedDateTime()}`;
          setFlipbookName(newDefName);
          setInitialFlipbookName(newDefName);
          ...
          const newDefName = `PDF_Flipbook_${getFormattedDateTime()}`;
          setFlipbookName(newDefName);
          setInitialFlipbookName(newDefName);
          initialFlipbookNameRef.current = newDefName;
        }
