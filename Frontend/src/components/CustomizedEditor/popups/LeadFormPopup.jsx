    ...
    return (
        <div className="absolute inset-0 z-[100] bg-black/40 backdrop-blur-sm flex items-center justify-center p-[0.25vw]">
            <div className="relative" style={{ fontFamily: leadFormSettings.appearance.fontStyle || 'Inter' }}>
                ...
                >
                    <div className={`${isTablet ? 'p-[1vw] space-y-[0.6vw]' : 'p-[2vw] space-y-[1vw]'}`}>
                        {/* Header */}
                            <div className="space-y-[0.2vw]">
                            <div className={`flex items-center ${isTablet ? 'gap-[1vw]' : 'gap-[1vw]'} relative`}>
                                <h2
                                    className={`${isTablet ? 'text-[0.9vw]' : 'text-[1.3vw]'} font-semibold leading-none shrink-0`}
                                    style={{
                                        color: leadFormSettings.appearance.textFill || '#111827',
                                ...
                                {leadFormSettings.appearance.allowSkip ? (
                                    <button
                                        onClick={onClose}
                                        className={`${isTablet ? 'text-[0.5vw]' : 'text-[0.7vw]'} font-semibold text-blue-600 hover:text-blue-700 transition-colors flex-shrink-0 cursor-pointer`}
                                    >
                                        Skip
                                    </button>
                                ) : (
                            ...
                            <p
                                className={`${isTablet ? 'text-[0.45vw]' : 'text-[0.65vw]'} font-semibold`}
                                style={{
                                    color: leadFormSettings.appearance.textFill || '#1F2937',
                        ...
                        <div className={`${isTablet ? 'py-[0.1vw]' : 'py-[0.4vw]'} text-center`}>
                            <p
                                className={`${isTablet ? 'text-[0.6vw]' : 'text-[0.9vw]'} font-semibold`}
                                style={{
                                    color: leadFormSettings.appearance.textFill || '#111827',
                                    WebkitTextStroke: leadFormSettings.appearance.textStroke && leadFormSettings.appearance.textStroke !== '#' ? `0.02vw ${leadFormSettings.appearance.textStroke}` : 'none'
                                }}
                        ...
                        {/* Form Fields */}
                        <div className={`${isTablet ? 'space-y-[0.8vw] w-full max-w-[14vw]' : 'space-y-[1.1vw] w-full max-w-[18vw]'} mx-auto`}>
                            {(leadFormSettings.fields || []).map(field => {
                                if (field.type === 'feedback') {
                                    return (
                                        <div key={field.id} className="space-y-[0.6vw]">
                                            
                                            <textarea
                                                placeholder={field.placeholder || 'Enter your Feedback'}
                                                className={`w-full bg-white border ${isTablet ? 'rounded-[0.4vw] p-[0.8vw] text-[0.5vw] h-[6vw]' : 'rounded-[0.6vw] p-[1vw] text-[0.7vw] h-[8vw]'} font-medium focus:outline-none transition-all resize-none shadow-sm`}
                                                style={{
                                                    borderColor: leadFormSettings.appearance.bgStroke || '#D1D5DB',
                                                    color: leadFormSettings.appearance.textFill || '#111827',
                                                    fontFamily: 'inherit'
                                ...
                                return (
                                    <div key={field.id} className="relative">
                                        <div className={`${isTablet ? 'left-[0.6vw]' : 'left-[0.9vw]'} absolute top-1/2 -translate-y-1/2 ${field.type === 'phone' ? 'text-gray-400' : 'text-gray-900'}`}>
                                            {field.type === 'email' ? <Icon icon="logos:google-gmail" className={`${isTablet ? 'w-[1vw] h-[1vw]' : 'w-[1.1vw] h-[1.1vw]'}`} /> :
                                             field.type === 'phone' ? <Icon icon="lucide:phone" className={`${isTablet ? 'w-[1vw] h-[1vw]' : 'w-[1.1vw] h-[1.1vw]'}`} /> :
                                             <Icon icon="lucide:user" className={`${isTablet ? 'w-[1vw] h-[1vw]' : 'w-[1.1vw] h-[1.1vw]'}`} />}
                                        </div>
                                        <input
                                            type={field.type === 'email' ? 'email' : field.type === 'phone' ? 'tel' : 'text'}
                                            placeholder={field.placeholder || `Enter your ${field.type}`}
                                            className={`w-full bg-white border ${isTablet ? 'rounded-[0.4vw] py-[0.4vw] pl-[2.2vw] pr-[0.8vw] text-[0.5vw]' : 'rounded-[0.6vw] py-[0.6vw] pl-[3.1vw] pr-[0.9vw] text-[0.7vw]'} font-medium focus:outline-none transition-all shadow-sm`}
                                            style={{
                                                borderColor: leadFormSettings.appearance.bgStroke || '#D1D5DB',
                                                color: leadFormSettings.appearance.textFill || '#111827',
                                                fontFamily: 'inherit'
